...
interface CropSuitability {
    ...
    reason: string;
    moistureMatch: number; // 0-1 score
    soilMatch: boolean;
    seasonMatch: number; // 0-1 score
}
...
/**
 * Soil moisture requirements for UP-valid crops (VWC %)
 * Based on sandy loam soil in UP alluvial plains
 * Source: Field capacity ~35-45%, Wilting point ~8-10%
 */
interface MoistureRange {
    min: number; // Minimum acceptable VWC
    optimal: number; // Optimal VWC for best growth
    max: number; // Maximum VWC (above = waterlogging risk)
}

const CROP_MOISTURE_REQUIREMENTS: { [key: string]: MoistureRange } = {
...
const CROP_SOIL_SUITABILITY: { [key: string]: string[] } = {
 ...
 */
const CROP_SEASON_SUITABILITY: { [key: string]: { [key: string]: number } } = {
 ...
 */
export async function recommendCropsForUP(
    nodeId: number,
    ...
    try {
        ...
        const avgSoilMoisture =
            recentReadings.reduce((sum, r) => sum + (r.soilMoistureVWC || 0), 0) /
            recentReadings.length;

        const avgSoilTemp =
            recentReadings.reduce((sum, r) => sum + (r.soilTemperature || 0), 0) /
            recentReadings.length;

        console.log(
            `[CropRec] Avg (${recentReadings.length} readings): ${avgSoilMoisture.toFixed(2)}% VWC, ` +
        ...
        for (const cropName of UP_VALID_CROPS) {
            ...
            if (moistureMatch > 0.8) {
                ...
                reasons.push(
                    `Poor soil moisture (${avgSoilMoisture.toFixed(1)}% VWC, needs ${moistureReq.min}-${moistureReq.max}%)`
                );
            }
        ...
        console.log(
            `[CropRec] ✅ Top recommendation: ${bestCrop.cropName} (${bestCrop.suitability}%)`
        );
 ...
 */
function calculateMoistureMatch(actualVWC: number, requirement: MoistureRange): number {
...
function generateSummary(
    bestCrop: CropSuitability,
    alternatives: CropSuitability[],
    season: string
): string {
...
export function filterUPCrops(crops: string[]): string[] {
    return crops.filter((crop) => {
        ...
        return (UP_VALID_CROPS as readonly string[]).includes(normalized);
    });
}
