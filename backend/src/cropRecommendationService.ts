...
type CropName = 'wheat' | 'rice' | 'vegetables' | 'bajra';
...
const cropRequirements: Record<CropName, CropRequirement> = {
    wheat: {
        moistureMin: 350,
        moistureMax: 700,
        moistureIdeal: 500,
...
export function recommendCrop(
    ...
    const cropNames: CropName[] = ['wheat', 'rice', 'vegetables', 'bajra'];
    const crops: CropSuitability[] = cropNames.map(cropName => {
        const suitability = calculateSuitability(moisture, temperature, cropName);

        let reason: string;
        if (suitability > 80) reason = 'Excellent conditions';
        else if (suitability > 60) reason = 'Good conditions';
        else if (suitability > 40) reason = 'Moderate conditions';
        else reason = 'Poor conditions';
