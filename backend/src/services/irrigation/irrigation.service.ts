...
import type { SoilTexture, UPCropName, GrowthStage, IrrigationUrgency } from '../../utils/constants.js';
...
function calculateIrrigationDepth(
    ...
    const depth = Math.max(
        ...
        Math.min(deficit, IRRIGATION_CONSTANTS.MAX_IRRIGATION_DEPTH_MM)
    );
...
export async function makeIrrigationDecision(nodeId: number): Promise<IrrigationDecision> {
    try {
        ...
        logger.info(
            { nodeId, decision, urgency: finalUrgency, currentVWC, targetVWC },
