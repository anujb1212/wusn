    ...
    try {
        ...
        if (soilMoistureVWC === 0 && soilTemperature === 0) {
            throw new ValidationError(
                `Sensor zero readings for nodeId=${payload.nodeId}. ` +
 ...
 */
export async function getLatestSensorData(nodeId: number) {
    try {
 ...
 */
export async function getAverageSoilData(nodeId: number, hours: number = 24) {
    try {
 ...
 */
export async function getAverageAirData(nodeId: number, hours: number = 24) {
    try {
...
async function _triggerAggregation(
    ...
    const readings = await Promise.all(
        nodes.map(n => getLatestReading(n.nodeId))
    );
    ...
    await prisma.aggregatedReading.create({
        data: {
            ...
            selectionReason:
                `Node ${bestNodeId} selected — highest reliability score ` +
    ...
    logger.info(
        { selectedNodeId: bestNodeId, totalNodes: snapshots.length, score: bestScore },
        'Aggregation snapshot written'
    );
}
...
function _computeNodeScore(reading: {
    ...
    if (vwc <= 0 || vwc > 90) return 0;

    const vwcScore = Math.max(0, 100 - Math.abs(vwc - 25) * 2);

    const tempScore =
        reading.soilTemperature >= 10 && reading.soilTemperature <= 45
            ? 100
            : 50;
