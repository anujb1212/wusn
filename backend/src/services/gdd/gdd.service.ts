...
import type { GrowthStage, UPCropName } from '../../utils/constants.js';
 ...
 * Calculate daily GDD
 */
function calculateDailyGDDValue(avgAirTemp: number, baseTemp: number): number {
...
export async function calculateDailyGDD(
    ...
    try {
        ...
        const growthStage = field.expectedGDDTotal && field.cropType
            ? determineGrowthStage(cumulativeGDD, field.expectedGDDTotal, field.cropType as UPCropName)
        ...
        logger.info(
            { nodeId, date: dateOnly, dailyGDD, cumulativeGDD, growthStage },
            'GDD calculated'
        );
