...
export interface CropKcValues {
    ini: number;  // Initial stage Kc
    mid: number;  // Mid-season stage Kc
    end: number;  // Late season stage Kc
}
...
export interface CropParameters {
    ...
    baseTemp: number;           // Tbase for GDD calculation: GDD = max(0, (Tmax+Tmin)/2 - Tbase)
    soilTempMin: number;        // Min soil temperature for germination/growth
    soilTempOptimal: number;    // Optimal soil temperature
    soilTempMax: number;        // Max soil temperature before stress

    // Volumetric Water Content (VWC %)
    vwcMin: number;             // Min VWC before stress
    vwcOptimal: number;         // Optimal VWC for growth
    vwcMax: number;             // Max VWC before waterlogging

    // Irrigation parameters
    rootDepthCm: number;        // Effective root zone depth (cm)
    mad: number;                // Management Allowed Depletion (0-1)
    kc: CropKcValues;           // FAO-56 crop coefficients

    // GDD thresholds for growth stages
    initialStageGDD: number;    // GDD to reach 10% ground cover
    developmentStageGDD: number; // GDD to reach full cover
    midSeasonGDD: number;       // GDD at full productivity
    lateSeasonGDD: number;      // GDD at senescence/maturity
 ...
 */
export const CROP_DATABASE: Record<CropName, CropParameters> = {
    // ========================
    // FIELD CROPS / STAPLES
    // ========================
