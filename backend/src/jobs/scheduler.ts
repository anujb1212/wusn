 ...
 */
export function startGDDCalculationJob(): void {
    ...
    cron.schedule(cronExpression, async () => {
        ...
        try {
            ...
            const results = await Promise.allSettled(
                fields.flatMap(f => f.nodes.map(n => calculateMissingGDD(n.nodeId)))
            );
            ...
            logger.info(
                ...
                'Daily GDD calculation job completed'
            );
        } catch (error) {
 ...
 */
export function startWeatherCacheJob(): void {
    ...
    cron.schedule(cronExpression, async () => {
        ...
        try {
            ...
            logger.info(
                { total: locationMap.size, success: successCount, failed: failCount },
                'Weather cache refresh job completed'
            );
        } catch (error) {
