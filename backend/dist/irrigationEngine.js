...
import { getLatestGDDStatus } from './gddService.js';
...
export async function makeIrrigationDecision(nodeId, weatherData) {
    try {
        ...
        const growthStage = gddStatus.latestGDD?.growthStage || 'INITIAL';
        console.log(`[Irrigation] Crop: ${cropType}, Stage: ${growthStage}, Current VWC: ${currentVWC.toFixed(2)}%`);
        ...
        console.log(`[Irrigation] Target: ${targetVWC.toFixed(1)}%, RAW: ${RAW_threshold.toFixed(1)}%, Critical: ${criticalVWC.toFixed(1)}%`);
        // RULE 1: CRITICAL
        if (currentVWC <= criticalVWC) {
            const waterNeeded = calculateWaterNeeded(currentVWC, targetVWC, cropType, growthStage);
            await logIrrigationDecision(nodeId, true, 'CRITICAL', currentVWC, targetVWC, waterNeeded, cropType, growthStage);
            return {
                shouldIrrigate: true,
        ...
        if (currentVWC <= RAW_threshold) {
            const waterNeeded = calculateWaterNeeded(currentVWC, targetVWC, cropType, growthStage);
            await logIrrigationDecision(nodeId, true, 'HIGH', currentVWC, targetVWC, waterNeeded, cropType, growthStage);
            return {
                shouldIrrigate: true,
        ...
        if (currentVWC < targetVWC && isCriticalStage) {
            const waterNeeded = calculateWaterNeeded(currentVWC, targetVWC, cropType, growthStage);
            await logIrrigationDecision(nodeId, true, 'MEDIUM', currentVWC, targetVWC, waterNeeded, cropType, growthStage);
            return {
                shouldIrrigate: true,
...
function getCropMoistureRequirement(cropType, growthStage) {
...
function calculateWaterNeeded(currentVWC, targetVWC, cropType, growthStage) {
...
function getRootZoneDepth(cropType, growthStage) {
    const depths = {
        rice: {
            INITIAL: 15,
            DEVELOPMENT: 30,
...
function getRecommendedMethod(cropType, growthStage) {
    const normalizedCrop = cropType.toLowerCase().replace(/\s+/g, '');
...
function getRainForecast(weatherData) {
    let totalRain = 0;
    const days = 3;
...
async function logIrrigationDecision(nodeId, shouldIrrigate, urgency, currentVWC, targetVWC, estimatedWaterNeeded, cropType, growthStage) {
    try {
...
export async function recordIrrigationAction(nodeId, waterAppliedMm) {
    try {
