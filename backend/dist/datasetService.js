...
class DatasetService {
     ...
     */
    calculateCropSuitability(moisture, temperature, cropName) {
        ...
        if (temperature >= range.tempMin && temperature <= range.tempMax) {
            ...
            const maxDeviation = Math.max(range.tempIdeal - range.tempMin, range.tempMax - range.tempIdeal);
            tempScore = 100 * (1 - deviation / maxDeviation);
        }
        else {
            // Penalty for out of range
            const outOfRangeDistance = Math.min(Math.abs(temperature - range.tempMin), Math.abs(temperature - range.tempMax));
        ...
        if (estimatedHumidity >= range.humidityMin && estimatedHumidity <= range.humidityMax) {
            ...
            const maxDeviation = Math.max(range.humidityIdeal - range.humidityMin, range.humidityMax - range.humidityIdeal);
            humidityScore = 100 * (1 - deviation / maxDeviation);
        }
        else {
            const outOfRangeDistance = Math.min(Math.abs(estimatedHumidity - range.humidityMin), Math.abs(estimatedHumidity - range.humidityMax));
     ...
     */
    getSeasonalCrops(temperature) {
        // Filter only crops that exist in dataset
        const availableCrops = this.northIndiaCrops.filter(crop => this.cropRanges.has(crop));
        // Seasonal filtering for North India
        if (temperature < 25) {
            // Rabi season (Oct-April): Cool season crops
            const rabiCrops = availableCrops.filter(c => ['wheat', 'chickpea', 'lentil', 'mothbeans', 'pomegranate'].includes(c));
            ...
            const kharifCrops = availableCrops.filter(c => ['rice', 'maize', 'cotton', 'mungbean', 'pigeonpeas',
     ...
     */
    getCropIdealConditions(cropName) {
