...
function convertSMUtoVWC(smu) {
    ...
    const vwc = ((clampedSMU - SMU_MIN) / (SMU_MAX - SMU_MIN)) * (VWC_MAX - VWC_MIN) +
        VWC_MIN;
 ...
 */
function convertTemperatureToCelsius(rawTemp) {
...
async function handleSensorData(payload) {
    try {
        ...
        console.log(`[MQTT] 📥 Received from Node ${nodeId}: ` +
        ...
        console.log(`[MQTT] 🔄 Converted: SMU ${moisture} → ${soilMoistureVWC.toFixed(2)}% VWC, ` +
        ...
        console.log(`[MQTT] ✅ Stored reading ID ${reading.id}: ` +
 ...
 */
async function checkAndCalculateGDD(nodeId) {
    try {
        ...
        console.log(`[MQTT] 🌱 Triggering GDD calculation for yesterday (${yesterdayStart.toISOString().split('T')[0]})`);
        // Run GDD calculation asynchronously (don't block MQTT processing)
        calculateDailyGDDFromSoilTemp(nodeId, yesterdayStart).catch((err) => {
 ...
 */
export function publishAlert(nodeId, alertType, message) {
    if (mqttClient && mqttClient.connected) {
        const alertData = {
            type: 'alert',
            nodeId,
        ...
        mqttClient.publish(`wusn/alerts/${nodeId}`, JSON.stringify(alertData), { qos: 2, retain: true }, // QoS 2 for critical alerts
        (err) => {
            if (err) {
