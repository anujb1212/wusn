 ...
 */
function calculateGDDValue(minAirTemp, maxAirTemp, baseTemp, upperThreshold = UPPER_TEMP_THRESHOLD) {
...
export async function calculateDailyGDD(nodeId, date) {
    try {
        ...
        if (!field.cropConfirmed || !field.sowingDate || !field.baseTemperature || !field.cropType) {
            logger.warn({ nodeId, cropConfirmed: field.cropConfirmed, hasSowingDate: !!field.sowingDate }, 'Field not configured for GDD calculation');
            return null;
        }
        ...
        if (tempData.minAirTemp == null || tempData.maxAirTemp == null) {
            logger.warn({ nodeId, date: dateOnly.toISOString().split('T')[0], tempData }, 'Missing min/max air temperature, cannot calculate GDD');
            return null;
        }
...
export async function recalculateGDDRange(nodeId, startDate, endDate) {
    try {
        ...
        logger.info({ nodeId, calculatedCount, requestedDays: Math.floor((endDateOnly.getTime() - startDateOnly.getTime()) / (1000 * 60 * 60 * 24)) + 1 }, 'GDD recalculation complete');
        return calculatedCount;
    }
    catch (error) {
        logger.error({ error, nodeId, startDate: startDate.toISOString(), endDate: endDate.toISOString() }, 'Failed to recalculate GDD range');
        throw error;
    }
}
/**
...
export async function calculateMissingGDD(nodeId) {
    try {
        ...
        const existingDates = new Set(existingRecords.map(r => {
        ...
        logger.info({ nodeId, missingCount: missingDates.length, dateRange: `${missingDates[0]?.toISOString().split('T')[0]} to ${missingDates[missingDates.length - 1]?.toISOString().split('T')[0]}` }, 'Calculating missing GDD records');
        ...
        logger.info({ nodeId, calculatedCount, missingCount: missingDates.length }, 'Missing GDD calculation complete');
        return calculatedCount;
    }
    catch (error) {
