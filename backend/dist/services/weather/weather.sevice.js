...
import { cacheWeatherForecast, getCachedWeatherForecast, } from '../../repositories/weather.repository.js';
...
async function fetchFromAPI(latitude, longitude) {
    try {
        ...
        const forecast = Array.from(dailyData.entries())
        ...
        // Cache in database
        await cacheWeatherForecast(latitude, longitude, forecast, WEATHER_CONSTANTS.CACHE_TTL_HOURS);
 ...
 */
export async function estimateDailyET(latitude, longitude) {
    try {
        const forecast = await getWeatherForecast(latitude, longitude);
...
//# sourceMappingURL=weather.sevice.js.map