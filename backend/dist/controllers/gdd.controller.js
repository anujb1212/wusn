 ...
 */
export async function recalculateGDDController(req, res) {
    const { nodeId } = nodeIdSchema.parse(req.params);
    const { startDate, endDate } = dateRangeSchema.parse(req.body);
    const count = await recalculateGDDRange(nodeId, new Date(startDate), new Date(endDate));
    res.json({
        status: 'ok',
        data: {
            nodeId,
...
//# sourceMappingURL=gdd.controller.js.map