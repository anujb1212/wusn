 ...
 */
export async function getDailyAirTempByNode(nodeId, date) {
    try {
        const startOfDay = new Date(date);
        startOfDay.setHours(0, 0, 0, 0);
        const endOfDay = new Date(date);
        ...
        throw new DatabaseError('getDailyAirTempByNode', error);
    }
}
/**
 ...
 */
export async function getAirTempReadingsForDateRange(nodeId, startDate, endDate) {
    try {
        const readings = await prisma.sensorReading.findMany({
            where: {
                nodeId,
        ...
        throw new DatabaseError('getAirTempReadingsForDateRange', error);
    }
}
// ============================================================================
    ...
    try {
        ...
        return await prisma.weatherForecast.create({
            data: {
                latitude,
                longitude,
                forecastData: forecastData, // JSON type in Prisma
                fetchedAt: now,
                expiresAt,
            },
        });
